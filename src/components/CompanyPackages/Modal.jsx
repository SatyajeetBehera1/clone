...
const ModalImage = ({ src, alt, isFirst }) => (
  <div className={`relative ${isFirst ? 'col-span-2 row-span-2' : 'col-span-1 row-span-1'} group overflow-hidden rounded-lg shadow-lg transition duration-300 ease-in-out mt-2`}>
    <img
      src={src}
      alt={alt}
      className={`w-full h-full object-cover ${isFirst ? 'mt-0' : 'mt-4 sm:mt-0'}`} 
...
export default function Modal({ onClose }) {
  ...
  // Function to chunk images into groups of 5
      ...
      <div className="relative w-full h-full">
        ...
        <div className="relative bg-white rounded-lg shadow-lg w-full h-full flex flex-col p-8">
          ...
          <div className="p-4 md:p-5 flex-grow overflow-y-auto scrollbar scrollbar-thumb-white">
            {/* Render all images grouped */}
            {groupedImages.map((group, groupIndex) => (
              <div key={groupIndex} className="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 mt-8">
                {group.map((image, index) => (
                  <ModalImage key={index} src={image.src} alt={image.alt} isFirst={index === 0} />
                ))}
              </div>
            ))}
          </div>
